            ...
            return (
              ...
              >
                ...
                <label className="flex items-center gap-2 mt-2 cursor-pointer">
                  <div
                    onClick={() =>
                      updateContact(contact.id, "active", !contact.active)
                    }
                    className={`w-10 h-5 rounded-full transition-colors relative ${contact.active ? "bg-brand-blue" : "bg-brand-gray-300"}`}
                  >
                    <div
                      className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all ${contact.active ? "left-5" : "left-0.5"}`}
                    />
                  </div>
                  <span className="text-xs text-brand-gray-500">
                    {contact.active ? "Actif" : "Masqué"}
                  </span>
                </label>
              </div>
            );
